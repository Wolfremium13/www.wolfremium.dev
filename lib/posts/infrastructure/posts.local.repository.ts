...
export class PostsLocalRepository implements PostsRepository {
  ...
  async getPaginatedPosts(pageNumber: number): Promise<Post[]> {
    ...
    return new Promise((resolve) => resolve(posts.slice(startIndex, endIndex)));
  }
  ...
  async getAllFilePaths(): Promise<string[]> {
    const filenames = fs
      ...
      .filter((fn) => fn.endsWith(this.fileFormat))
      .map((filename) => {
        const fileNameWithoutExtension = filename.replace(this.fileFormat, "");
  ...
  async getPosts(): Promise<Post[]> {
    ...
    return new Promise((resolve) => resolve(sortedPosts));
  }
}
