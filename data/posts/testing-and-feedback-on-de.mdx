...
sabemos implementar la pirámide de los test invertida. El tiempo es algo
superimportante tanto en la vida como en el trabajo, ¿no sería maravilloso poder
salir antes debido a un buen trabajo realizado?
...
def test_some_behavior_of_your_transformation(self):
    input = read_from_a_json_file()
    expected = read_from_a_json_file()

    result = self.calculator.do_some_stuff(input)
...
def test_some_behavior_of_your_transformation(self):
    input = generate_data_using_a_builder(value=True)
