...
export const MarkdownEditorInput: React.FC<Props> = ({
    ...
    const handleUploadImage: UploadImgEvent = async (files: File[], callBack: UploadImgCallBack) => {
        if (files && files[0]) {
            ...
            if (fileToUpload.size > 5 * 1024 * 1024 && SERVER_URL.current !== "http://localhost:3000") {
                alert("El archivo es demasiado grande, por favor elige un archivo de menos de 5MB")
            } else {
                if (fileToUpload) {
